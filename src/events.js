...
client.on("channelCreate", function(channel){
    console.log(channel.id + ' has been created its name is ' + channel.name)
});
...
client.on("guildUnavailable", function(guild){
    console.error(`a guild becomes unavailable, likely due to a server outage: ${guild.id}`);
});
...
client.on("roleDelete", function(role){
    console.error(`${role.name}a guild role is deleted`);
});
...
client.on("roleUpdate", function(oldRole, newRole){
    console.error(`${oldRole.name}, a guild role is updated`);
});
...
client.on("userUpdate", function(oldUser, newUser){
    console.log(`user's details (e.g. username) are changed ${oldUser}`);
});
