...
module.exports = class Shadow extends Client {
  constructor(ops = {}) {
    ...
    this.error = async function (error, type) {
      ...
      try {
        ...
        const embed = new Discord.MessageEmbed()
          ...
          .setColor("#ff0000")
          .setFooter(
            `with in total of ${await await this.db.get(
              "errors"
            )} and in this session error count of ${
              this.errorCount
            } type: ${type}`
          );
        const m = await this.channels.cache
          .get("829753754713718816")
          .send({ embeds: [embed] });
        await this.db.set("error_" + m.id, {
          errorcount: {
            cache: await this.db.get("errors"),
            startup: this.errorCount,
          },
          date: Date.now(),
        });
      } catch (e) {
    ...
    this.on("interaction", async (interaction, op2) => {
      ...
      if (!this.slash_commands.get(cmd))
        return interaction.send("cannot get command " + cmd, {
          ephemeral: true,
        });
