...
class ARemoteDB extends events {
   constructor(ops = dops) {
        ...
        this.ops = ops
        if(!ops.reciver) {
            const prams = [{ v: ops.uri, n: 'URI'},{ v: ops.username, n: 'USERNAME'},{v: ops.password, n: 'PASSWORD'}] 
            prams.forEach(pram => {
...
this.fetch =  async function(uri, prams) {
  if(this.connection) {
      ...
      this.connection.emit('debug', '[REQUEST] => request is being sent to ' + uri)
  }
  this.emit('debug', '[REQUEST] => sending request to ' + uri)
    return await require('node-fetch')(uri, prams)
}
this.headers = { username: ops.username, password: ops.password, 'Content-type': 'application/json', 'from': 'fetch@shadow'}
this.ping = 0
...
this.ping = Date.now() - date + message
    ...
    this.emit('debug', '[PING] => connected at a rate of ' + this.ping + "ms")
    if(this.connection) {
        ...
        this.connection.emit('debug', '[PING] => connected at a rate of ' + this.connection.ping + "ms")
...
async get(key,table) {
    if(!table) table = 'main'
    let res;
await this.fetch(this.uri + `/${table}/callback?key=` + encodeURI(key) + `&table=${table}`, {
    method: 'get',
    headers: this.headers
       ...
       app.delete(`/${TABLE}/callback`, (req,res) => {
        if(!this.accounts.get(`${req.headers[`username`]}@${req.headers[`password`]}`)) return res.status(401).json({ status: 401, error: `INVALID_LOGIN`});
         const body = req.body
          ...
          if(!Table[body.key]) return res.status(404).json({ message: null, invalid: true})
          delete Table[body.key]
          fs.appendFileSync(`${this.path}/${body.table}/logs.txt`, `\n deleted ${body.key}`)
      ...
      res.status(200).json({ status: 200, message: ress || [] })
       ...
       app.get(`/${TABLE}/callback`, (req,res) => {
        if(!this.accounts.get(`${req.headers[`username`]}@${req.headers[`password`]}`)) return res.status(401).json({ status: 401, message: `INVALID_LOGIN`});
          ...
          if(this.Isclosed) return res.status(403).json({ error: `closed` })
         const object = JSON.parse(fs.readFileSync(this.path + `/${req.query.table}/${req.query.table}.json`))[req.query.key]
         if(req.query.raw) {
       ...
       this.emit('debug', `[SERVER] => loaded ${this.tables.length} table(s)`)
        this.server = app.listen(this.port, () => { this.emit('debug', '[SERVER] => server running') 
        this.emit('ready') })
    } else {
        let connection = await this.fetch(this.uri + '/connection', {
    headers: this.headers
}).catch(e => { try {console.error(e.message.bgRed) 
    this.emit('debug', `[CONNECTION/ERROR] => ${e.message}`)
} finally {  this.emit('debug', '[CONNECTION] => exiting... ') 
process.exit(1) } })
...
let list = {
    ping: 0,
    readyAt: Date.now(),
    uptime: Date.now() - this.connection.readyAt ? this.connection.readyAt : Date.now(),
    constroctur: {
...
async getRaw(key, table) {
    if(!table) table = 'main'
    await this.fetch(this.uri + `/${table}/callback?key=${encodeURIComponent(key)}&table=${table}&raw=true`, {
        headers: this.headers
