...
module.exports = class PhoneCall {
  ...
  async start() {
    ...
    await this.recipient.send(
      `**☎️ Incoming Call From ${this.origin.guild.name}! Pick up?**`
    );
  ...
  async accept() {
    ...
    this.active = true;
    this.setTimeout();
    await this.origin.send(
      `**☎️ ${this.recipient.guild.name} Picked Up! Type \`${prefix2}hangup\` To Hang Up!**`
    );
    await this.recipient.send(
      `**☎️ Accepted Call From ${this.origin.guild.name}! Type \`${prefix}hangup\` To Hang Up!**`
    );
    return this;
  }
  ...
  async hangup(nonQuitter, validation) {
    ...
    if (nonQuitter === "time") {
      ...
      const recipientMsg =
        validation === 0 ? "**Sent To Voicemail!**" : "**Declined The Call!**";
      ...
      if (validation === 0) {
        ...
        if (!voicemailValidation) {
          ...
          await this.origin.send(
            "**☎️ Please Leave Your Message!(max 280 characters)\nYou Have 30 Seconds!**"
          );

          const voicemail = await this.origin.awaitMessages(
            (res) => res.content && res.content.length <= 280,
            {
              max: 1,
              time: 30000,
            }
          ...
          if (!voicemail.size) {
            await this.origin.send(
              "**☎️ Timeout! No Voicemail Will Be Left!**"
            );
          } else {
        ...
        let originMsg =
          validation === 0 ? "**Didn't Answer!**" : "**Declined The Call!**";
        await this.origin.send(
          `**☎️ ${this.recipient.guild.name} ${originMsg}**`
        );
      }
    } else {
      const quitter =
        nonQuitter.id === this.origin.id ? this.recipient : this.origin;
  ...
  send(channel, msg, hasText, hasImage, hasEmbed) {
    if (this.cooldown.has(msg.author.id)) {
      const badChannel =
        channel.id === this.origin.id ? this.recipient : this.origin;
      return badChannel.send(
        `**☎️ ${msg.author}, Please Wait \`5\` Seconds Between Messages!**`
      );
    }
    this.setTimeout();
    ...
    const attachments = hasImage
      ...
      : null;
    if (!hasText && hasImage)
      return channel.send(`**☎️ ${msg.author.tag} - **\n${attachments}`);
    if (!hasText && hasEmbed)
      return channel.send(`**☎️ ${msg.author.tag} Sent An Embed!**`);
    let content = stripInvites(msg.content);
    content =
      content.length > 1000
        ? `${shorten(content, 500)} (Message too long)`
        : content;
    return channel.send(
      `☎️ **${msg.author.tag} -** ${content}\n${attachments || ""}`.trim()
    );
  }
