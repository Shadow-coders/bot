...
export default [
  {
    ...
    async execute(message: Message, args: String[], client: Shadow) {
      ...
      const btrow = new MessageActionRow()
        .addComponents(
          new MessageButton()
            .setCustomId(ID.FAST_BACK_PAGES)
            .setDisabled(true)
            .setEmoji("⏪")
            .setStyle("PRIMARY")
        )
        ...
        .addComponents(
          new MessageButton()
            .setCustomId(ID.FORWARD_PAGE)
            .setEmoji("▶️")
            .setStyle("PRIMARY")
        )
        .addComponents(
          new MessageButton()
            .setCustomId(ID.FAST_FORWARD_PAGES)
            .setStyle("PRIMARY")
            .setEmoji("⏩")
        );
      ...
      const msg = await message.reply({
        components,
        embeds: [pages[0]],
        content: `This collector will end in <t:${Math.round(
      ...
      const disable = (i?: any): Promise<void> => {
        return new Promise((resolve, reject) => {
          ...
          //@ts-ignore
          components[0].components.forEach((c: MessageButton) =>
      ...
      const collector = msg.channel.createMessageComponentCollector({
        ...
        filter: (i: any) => {
          ...
          if (!(i.user.id == message.author.id)) {
            i.reply({
              content: "You cant use these buttons or select menus!",
              ephemeral: true,
            });
            return false;
          }
