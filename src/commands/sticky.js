...
module.exports = [{
    ...
    async execute(message,args,client) {
      if(!message.member.permissions.has('MANAGE_GUILD')) return message.channel.send('missing perms')
        if(!args[0]) message.channel.send('I need a channel id')
        let findChannel = message.mentions.crosspostedChannels ? message.crosspostedChannels.first().id : args[0]
        if(args[0] === 'null') {
            findChannel = message.channel.id
        }
        const ch = client.channels.cache.get(findChannel) || client.channels.cache.get(args[0])
    if(!ch) return message.channel.send('Invalid channel ID or mention ')
    if(!args[1]) return message.channel.send('I need a message to make sticky') 
