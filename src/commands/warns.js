...
module.exports = [
  {
    ...
    async execute(message, args, client) {
      ...
      if (!warns.filter((t) => t.type === "warns"))
        return m.edit(user.username + " has No warns!");
  ...
  },
  {
    ...
    async execute(message, args, client) {
      ...
      if (!message.member.permissions.has("MANAGE_MESSAGES"))
        return message.reply("Missing Perms! | MannageMessages ");
      ...
      cases.push({
        ...
        id: SnowflakeUtil.generate(message.author.createdTimestamp),
      });
      user.send({
        embeds: [
          new MessageEmbed()
            .setTitle("Warned")
            .setDescription(
              `You have been warned fpr the reason of: ${args
                .slice(1)
                .join(" ")}\n`
            )
            .setColor("AQUA")
      ...
      m.edit({
        ...
        let { send } = (await client.db.get("modchannel_" + msg.guildId))
          ? await message.guild.channels.fetch(
              await client.db.get("modchannel_" + message.guildId)
            )
          : { send: () => {} };
        send({
          embeds: [
            new MessageEmbed()
              ...
              .setURL(m.url)
              .setDescription(
                `${user.tag} (${user.id}) has been warned for ${args
                  .slice(1)
