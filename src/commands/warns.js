...
user.send({ embeds: [new MessageEmbed().setTitle('Warned').setDescription(`You have been warned fpr the reason of: ${args.slice(1).join(' ')}\n`).setColor('AQUA').setTimestamp()]})
m.edit({ embeds: [new MessageEmbed().setAuthor(message.author.tag, message.author.displayAvatarURL()).setDescription(`${user.tag} was warned!`).setColor('DARK_RED').setTimestamp()]}).then(async msg => {
    client.db.set('cases_' + user.id, cases)
    setTimeout(msg.delete, 3000)
    let { send } = await client.db.get('modchannel_' + msg.guildId) ? await message.guild.channels.fetch(await client.db.get('modchannel_' + message.guildId)): { send: () => { } }
send({ embeds: [new MessageEmbed().setTimestamp(message.createdTimestamp).setTitle(`New warn`).setURL(m.url).setDescription(`${user.tag} (${user.id}) has been warned for ${args.slice(1).join(' ')} \n **channel**: ${message.channel.toString()} \n `).setFooter(m.id)]})
})
