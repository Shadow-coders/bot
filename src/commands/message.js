...
let { user } = member 
 const words = await client.db.get('hil_' + user.id) || []
  ...
  .setDescription(message.channel.messages.cache.map(m => `\`${m.createdAt.toString().slice(0, 15)}\` **${m.author.username}**: ${m.content} \n`).slice(message.channel.messages.cache.size - 5, message.channel.messages.cache.size).join('\n'), { long: true })
.setColor('GREEN')
