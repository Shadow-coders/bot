...
async execute(interaction:CommandInteraction, cmd: string, args:any[], client: Shadow): Promise<any>{
    ...
    if(target.id === (interaction.member as GuildMember)?.id) return interaction.reply({ ephemeral: true, content: `You cannot kick yourself!`});
    if(target.id === interaction.guild?.id) return interaction.reply({ ephemeral: true, content: `You cannot kick yourself!`});
