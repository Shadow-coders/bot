...
module.exports = {
  ...
  async execute(member, client) {
    ...
    let fullmsg = client.util.massreace(msg, [
      { word: /{user.name}/, replaced: member.user.name },
    ...
    member.guild.channels.fetch(ch).then((c) => {
      //  client.error(c)
      if (c) c.send(fullmsg);
    });
