...
export default [
  {
    ...
    async execute(channel:any, client:Shadow) {
      ...
      if(channel.type === 'GUILD_STAGE_VOICE') channel = (channel as StageChannel)
      if(channel.type === 'GUILD_CATEGORY') channel = (channel as CategoryChannel)
      if(channel.type === 'GUILD_NEWS') channel = (channel as NewsChannel)
      
      
      
      if (!channel.guild) return client.error ? client.error("New dm channel created"):null;
      const ch:any = ((await client.db.get("chlogs_" + channel.guild.id)) as TextChannel)
  ...
  },
  {
    ...
    async execute(channel:any, client:Shadow) {
      //console.log(channel);
      if (!channel) return client.error ? client.error("no channel provided") : null;
      ...
      const ch:any = (await client.db.get("chlogs_" + channel.guild.id))
        ...
        : null;
      if (!ch) return client.error ? client.error("debug: no channel found!"):null;
      /**
       * @returns {String}
       */
      let desc;
