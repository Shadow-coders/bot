...
console.log(m.id)
	const ping = client.ws.ping + Date.now() - date + Date.now() - m.createdTimestamp + (client.db.ping)
			interaction.editReply('> Pong ' + client.ws.ping + `(\`${ms(client.ws.ping)}\`) \n > latency: ` + `${Date.now() -date} (\`${ms(Date.now() - date)}\`)\n > edit latency: ${Date.now() - m.createdTimestamp} (\`${ms(Date.now() - m.createdTimestamp)}\`) \n > DB latency ${client.db.ping} (\`${ms(client.db.ping)}\`) \n > overall ping: \`${ping}\` (\`${ms(ping)}\`)` );},
type: "slash",
