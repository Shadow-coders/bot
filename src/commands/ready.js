...
setInterval(() => {
let m = client.channels.cache.get('830471074193080381').messages.cache.get('830471635589005312')
m.edit({ embeds: [new MessageEmbed().setDescription(`RAM Status:
 ...
 - Total Usage: ${os.loadavg().map(a => a.toFixed(1)).join("/")} (1m/5m/15m)
 - CPU Based: ${os.arch()}
...
`).setTitle('Server stats').setThumbnail(client.user.displayAvatarURL()).setColor('#2f3136').setTimestamp(), new MessageEmbed().setTitle('Client stats').setDescription(`• Users: ${client.users.cache.size}
• Servers: ${client.guilds.cache.size}
• Channels: ${client.channels.cache.size}
- ${client.channels.cache.filter(c => c.type === 'GUILD_CATEGORY').size} Categories
- ${client.channels.cache.filter(c => c.type === 'GUILD_TEXT' || c.type === 'GUILD_NEWS').size} Text
-  ${client.channels.cache.filter(c => c.type === 'GUILD_VOICE').size} Voice
- ${client.channels.cache.filter(c => c.type === 'GUILD_STAGE_VOICE').size} stages
- ${client.channels.cache.filter(c => c.type === 'GUILD_PUBLIC_THREAD').size}  threads
