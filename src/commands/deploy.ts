...
if(!client.slash_commands.find((c:any) => c.name == command)) return message.reply('Command ' + command + ' does not exist');
...
const command = args[0]
const isGlobal = args[1] ? args[1] === 'global' : false
let cmddata = isGlobal ? client.application.commands.cache.find((c:any) => c.name === command) : message.guild.commands.cache.find((c:any) => c.name === command)
