...
export default  {
  ...
  (  client.channels.cache.get("765669027552559149")as TextChannel)?.send({
    ...
    (client.channels.cache
      .get("881552094610460682") as TextChannel)
      .send(
        `Bruh I am online bois come and test me <@${client.devs.join("> <@")}>`
      );
   ( client.channels.cache
      .get("832694631459192903") as NewsChannel)
      .send({
        embeds: [
          new MessageEmbed()
            .setTitle("Ready")
            ...
            .setThumbnail(
              client.guilds.cache.get("778350378445832233") ? `${client.guilds.cache.get("778350378445832233")?.iconURL()}` : ''
            )
            .setDescription(
