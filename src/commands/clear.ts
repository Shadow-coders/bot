import { Shadow, Message, CommandInteraction, MessageEmbed, TextChannel } from '../client'
export default [
    {
name: 'clear',
...
if(Target) {
    ...
    (await messages)?.filter((m) => {
        //@ts-expect-error
        if(m.author.id === Target.user.id 
            && (Amount ? Amount : 0) > i) {
            filtered.push(m)
            i++;
        }
   })
   await (channel as TextChannel).bulkDelete(filtered, true).then((messages) => {
    Res.setDescription(`🧹 Cleared \`${messages.size}\` from ${Target}.`);
    interaction.reply({
        embeds: [Res]
    });
    ...
    await (channel as TextChannel).bulkDelete(Amount, true).then((messages) => {
        Res.setDescription(`🧹 Cleared \`${messages.size}\` from this channel`);
        interaction.reply({
            embeds: [Res]
        });
    })
